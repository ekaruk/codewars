...
/*
    public static String decodeBitsAdvanced2(String bits) {
    	...
    	if (bit1[0].length() == bit1[bit1.length-1].length()) 
    	  {
    		double unitLength = bit1[0].length();
    	  }
    	else  
    	  {
        	double num1 = bits.replaceAll("0", "").length();
        	double unitLength = (1.1)*num1/bit1.length;
    	  }
    	...
    	{  if (word.contains("0")) 
    	    {
    		if (word.length() > (unitLength*2.33)) {curr = "   ";}
    		else if (word.length() > (unitLength)) {curr = " ";}
    		else {curr = "";}
    	    }
    	else 
    	    {
    		if (word.length() > (unitLength)) {curr = "-";}
    		else {curr = ".";}
    	    }
    	result.append(curr);  
    	}
    	System.out.println("result:"+unitLength+":"+result.toString());
